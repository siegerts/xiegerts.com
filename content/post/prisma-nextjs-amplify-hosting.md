...
For now, we'll create a basic schema with a `Post` table that has a few attributes.
...
You need to update the `DATABASE_URL` environment variable to point to a PostgreSQL compatible database. This database can be different from what you'll use when you deploy the application. Typically, you will use a different database for each environment (local, test, prod, etc.). 
